...
<nav x-data="{ open: false }" class="bg-white border-b border-slate-200">
                    ...
                    <x-dropdown align="right" width="48">
                        <x-slot name="trigger">
                            <button class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-slate-600 bg-white hover:text-slate-800 focus:outline-none transition ease-in-out duration-150">
                                <div x-data="{ name: @json(auth()->user()?->name ?? 'Utilisateur') }" x-text="name" x-on:profile-updated.window="name = $event.detail.name"></div>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        ...
        <div class="pt-4 pb-1 border-t border-slate-200">
            ...
            @else
                <div class="px-4">
                    <div class="font-medium text-base text-slate-800">Invité</div>
                    <div class="font-medium text-sm text-slate-500">Connectez-vous pour accéder au tableau de bord</div>
                </div>
                <div class="mt-3 space-y-1">
